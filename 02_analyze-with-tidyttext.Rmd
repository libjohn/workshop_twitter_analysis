...
search_tweets
```{r getTweets}
tweet_collection <- search_tweets("#nfl", n=4000, lang = "en")
...
```{r word_freq plot}
ggplot(frequency, aes(CBBCent1, STLBreak)) +
...
word_ratios <- tweets_tokenized %>%
  ...
  filter(screen_name == "CBBCent1" | screen_name == "STLBreak") %>% 
  filter(!str_detect(word, "^@")) %>%
  ...
  mutate(logratio = log(CBBCent1 / STLBreak)) %>%
...
word_ratios %>%
  ...
  coord_flip() +
  ylab("log odds ratio (STLBreak/CBBCent1)") +
  scale_fill_discrete(name = "", labels = c("STLBreak", "CBBCent1"))
```
